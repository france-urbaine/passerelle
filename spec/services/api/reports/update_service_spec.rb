...
RSpec.describe API::Reports::UpdateService do
  subject(:service) do
    described_class
  end
  ...
  context "when report is incomplete and updated with complete form" do
    it "save the report" do
      expect { service.new(report, attributes).save }.to change(report, :updated_at)
    end
  end

  context "when report is incomplete and updated with incomplete form" do
    it "doesn't save the report" do
      expect { service.new(report, incomplete_attributes).save }.not_to change(report, :updated_at)
    end
  end
