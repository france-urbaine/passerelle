...
RSpec.describe UI::CodeRequestExample::Component do
  ...
  context "without optional arguments" do
    ...
    it "renders the correct cURL command" do
      expect(page).to have_code_in_panel("cURL") do |code|
        expect(code).to have_selector("span.code-example__block > span") do |span|
    ...
    it "renders the correct httpie command" do
      expect(page).to have_code_in_panel("httpie", visible: :hidden) do |code|
        expect(code).to have_selector("span.code-example__block > span", visible: :hidden) do |span|
          expect(span).to have_html_attribute("data-highlight-language-value").with_value("shell")
          expect(span).to have_text(<<~TEXT.strip, exact: true)
  ...
  context "with custom headers" do
    ...
    it "renders the correct cURL command" do
      expect(page).to have_code_in_panel("cURL", visible: :visible, text: <<~TEXT.strip)
        $ curl -X GET http://api.test.host/path \\
