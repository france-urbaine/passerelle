...
RSpec.describe "Reports::ApprovalsController#destroy" do
  subject(:request) do
    delete "/signalements/#{report.id}/approval", as:, headers:, params:
  end
      ...
      context "when signed in as a ddfip user" do
        ...
        it "change state and not change approved_at" do
          expect { request and report.reload }
            .to  change(report, :state).from("approved").to("processing")
    ...
    context "when report is rejected" do
      ...
      context "when signed in as a ddfip user" do
        ...
        it "change state and not change rejected_at" do
          expect { request and report.reload }
            .to  change(report, :state).from("rejected").to("processing")
