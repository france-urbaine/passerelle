...
RSpec.describe "Reports::RejectionsController#update" do
  subject(:request) do
    patch "/signalements/#{report.id}/rejection", as:, headers:, params:
  end
  ...
  describe "responses" do
    context "when report is assigned" do
      ...
      context "when signed in as a ddfip user" do
        ...
        it "change state and fill rejected_at" do
          expect { request and report.reload }
            .to  change(report, :state).from("processing").to("rejected")
    ...
    context "when report is already rejected" do
      ...
      context "when signed in as a ddfip user" do
        ...
        it "do not change state and rejected_at" do
          expect { request and report.reload }
      ...
      context "when signed in as a ddfip admin" do
        ...
        it "change state and not change rejected_at" do
          expect { request and report.reload }
            .to  not_change(report, :state)
            .and not_change(report, :rejected_at)
        end
    ...
    context "when report is approved" do
      ...
      context "when signed in as a ddfip user" do
        ...
        it "change state and fill rejected_at" do
          expect { request and report.reload }
            .to  change(report, :state).from("approved").to("rejected")
            .and change(report, :rejected_at).to be_present
        end
      end
