...
RSpec.describe "Reports::ResolutionsController#show" do
  subject(:request) do
    get "/signalements/resolve/#{report.id}", as:, headers:, params:
  end

  let(:as)      { |e| e.metadata[:as] }
  let(:headers) { |e| e.metadata[:headers] }
  let(:params)  { |e| e.metadata.fetch(:params, { state: "applicable" }) }
