  ...
  describe "responses" do
    ...
    context "when signed in as an organization admin" do
      ...
      context "with valid attributes" do
        ...
        it "ignores the attributes to assign an organization and assign the current organization" do
          request
          expect(User.last.organization).to eq(current_user.organization)
        end
      end

      context "when trying to send super_admin attribute" do
        let(:attributes) { super().merge(super_admin: true) }

        it "ignores the attributes to assign a super admin" do
          request
          expect(User.last).not_to be_super_admin
        end
      end
    end
  end
