...
RSpec.describe "API::ReportsController#create" do
  subject(:request) do
    post "/api/transmissions/#{transmission_id}/signalements", as:, headers:, params:
  end
  ...
  let(:transmission_id) { transmission.id }
  ...
  describe "responses" do
    ...
    context "with valid report parameters and valid completeness" do
      ...
      it "returns the new report ID" do
        request
        expect(response).to have_json_body(id: Report.last.id)
      end
    end
    ...
    context "with invalid report parameters" do
      ...
      before do
        ...
        allow(report).to receive(:valid?).and_return(false)
      end
