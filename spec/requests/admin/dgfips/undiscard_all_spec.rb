...
RSpec.describe "Admin::DGFIPsController#undiscard_all" do
  subject(:request) do
    patch "/admin/dgfips/undiscard", as:, headers:, params:
  end

  before do
    skip("Admin::DGFIPsController#undiscard_all is not available")
  end
  ...
  describe "responses" do
    ...
    context "with multiple ids" do
      ...
      it "undiscards the selected dgfips" do
        expect {
          request
          dgfips.each(&:reload)
        }.to change(dgfips[0], :discarded_at).to(nil)
          .and change(dgfips[1], :discarded_at).to(nil)
          .and not_change(dgfips[2], :discarded_at).from(be_present)
      end

      it "sets a flash notice" do
        expect(flash).to have_flash_notice.to eq(
          type:  "cancel",
          title: "La suppression des DGFIPs sélectionnées a été annulée.",
          delay: 3000
        )
      end
    end

    context "with ids from already undiscarded DGFIPs" do
