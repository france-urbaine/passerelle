...
RSpec.describe "Territories::Regions::DDFIPs#index" do
  subject(:request) do
    get "/territoires/regions/#{region.id}/ddfips", as:, headers:, params:, xhr:
  end
  ...
  let!(:ddfips)      { create_list(:ddfip, 2, departement: departement) }
  ...
  describe "responses" do
    ...
    context "when requesting Turbo-Frame", headers: { "Turbo-Frame" => "datatable-ddfips" }, xhr: true do
      context "when the publisher is accessible" do
        it { expect(response).to have_http_status(:success) }
        it { expect(response).to have_content_type(:html) }
        it { expect(response).to have_html_body.with_turbo_frame("datatable-ddfips") }
