...
RSpec.describe "Territories::EpcisController#update" do
  subject(:request) do
    patch "/territoires/epcis/#{epci.id}", as:, headers:, params:
  end

  let(:as)      { |e| e.metadata[:as] }
  let(:headers) { |e| e.metadata[:headers] }
  let(:params)  { |e| e.metadata.fetch(:params, { epci: updated_attributes }) }
  ...
  describe "responses" do
    ...
    context "with valid attributes" do
      ...
      it { expect(response).to redirect_to("/territoires/epcis") }

      it "updates the epci" do
        expect { request and epci.reload }
