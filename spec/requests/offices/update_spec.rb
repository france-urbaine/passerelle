...
RSpec.describe "OfficesController#update" do
  ...
  describe "responses" do
    ...
    context "when signed in as a DDFIP admin, owner of the office" do
      before { sign_in_as(:organization_admin, organization: office.ddfip) }

      context "with valid attributes" do
        it { expect(response).to have_http_status(:see_other) }
        it { expect(response).to redirect_to("/guichets") }
        it { expect { request and office.reload }.to change(office, :updated_at) }
      end
