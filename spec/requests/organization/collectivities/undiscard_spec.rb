...
RSpec.describe "Organization::CollectivitiesController#undiscard" do
  subject(:request) do
    patch "/organisation/collectivites/#{collectivity.id}/undiscard", as:, headers:, params:
  end
  ...
  let!(:collectivity) { create(:collectivity, :discarded, allow_publisher_management: true) }
  ...
  describe "authorizations" do
    ...
    context "when the collectivity is owned by but didn't allow to be managed by the current publisher" do
      let(:collectivity) { create(:collectivity, :discarded, publisher: current_user.organization, allow_publisher_management: false) }
    ...
    context "when the collectivity is owned by and allowed to be managed by the current publisher" do
      let(:collectivity) { create(:collectivity, :discarded, publisher: current_user.organization, allow_publisher_management: true) }
    ...
    context "when the collectivity is likely to send reports to current DDFIP" do
      ...
      let(:collectivity) { create(:collectivity, :discarded, territory: commune) }
