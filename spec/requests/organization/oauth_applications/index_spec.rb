    ...
    context "when requesting HTML" do
      ...
      it "returns only kept oauth_applications owned by current organization" do
        aggregate_failures do
          expect(response.body).to not_include(CGI.escape_html(oauth_applications[0].name))
          expect(response.body).to not_include(CGI.escape_html(oauth_applications[1].name))
          expect(response.body).to not_include(CGI.escape_html(oauth_applications[2].name))
          expect(response.body).to include(CGI.escape_html(oauth_applications[3].name))
          expect(response.body).to include(CGI.escape_html(oauth_applications[4].name))
        end
      end
    end
