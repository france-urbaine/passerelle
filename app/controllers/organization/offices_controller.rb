...
module Organization
  class OfficesController < ApplicationController
    ...
    def new
      @office = build_office
      @referrer_path = referrer_path || organization_offices_path
    end

    def create
      @office = build_office
      service = ::Offices::CreateService.new(@office, office_params, ddfip: current_organization)
      result  = service.save

      respond_with result,
        flash: true,
        location: -> { redirect_path || organization_offices_path }
    end
    ...
    def remove
      @office = find_and_authorize_office
      @referrer_path = referrer_path || organization_office_path(@office)
      @redirect_path = referrer_path unless referrer_path&.include?(organization_office_path(@office))
    end
    ...
    def destroy
      ...
      respond_with @office,
        flash: true,
        actions: undiscard_organization_office_action(@office),
        location: redirect_path || organization_offices_path
    end
    ...
    def undiscard
      ...
      respond_with @office,
        flash: true,
        location: redirect_path || referrer_path || organization_offices_path
    end

    def remove_all
      @offices = authorize_offices_scope
      @offices = filter_collection(@offices)
      @referrer_path = referrer_path || organization_offices_path(**selection_params)
    end
    ...
    def destroy_all
      ...
      respond_with @offices,
        flash: true,
        actions: undiscard_all_organization_offices_action,
        location: redirect_path || organization_offices_path
    end
    ...
    def authorize_offices_scope(as: :default)
      authorized(current_organization.offices, as:).strict_loading
    end
    ...
    def find_and_authorize_office(allow_discarded: false)
      office = current_organization.offices.find(params[:id])
