    ...
    class ShowEmailComponent < ApplicationViewComponent
      ...
      def call
        if @user.unconfirmed_email?
          disabled_email @user.unconfirmed_email, "arrow-path", t("user_email.unconfirmed_email")
        elsif @user.confirmed?
          mail_to @user.email
        elsif @user.access_locked?
          disabled_email @user.email, "lock-closed", t("user_email.locked")
        elsif @user.confirmation_period_expired?
          disabled_email @user.email, "no-symbol", t("user_email.confirmation_period_expired")
        else
          disabled_email @user.email, "envelope", t("user_email.unconfirmed_user")
        end
      end
